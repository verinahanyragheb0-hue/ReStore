...
function loadProductsFromDB() {
    ...
    .then(data => {
        if (data.success && data.products.length > 0) {
            ...
            // Add wishlist heart buttons after cards are loaded
            function tryAddWishlistButtons() {
                if (window.wishlistManager) {
                    window.wishlistManager.addWishlistButtons();
                    window.wishlistManager.updateWishlistButtons();
                } else {
                    setTimeout(tryAddWishlistButtons, 100);
                }
            }
            tryAddWishlistButtons();
        }
    })
...
document.addEventListener("DOMContentLoaded", () => {
      ...
      new Promise((resolve) => {
        ...
        .then((data) => {
          if (data.success) {
            ...
            showToast(data.message || "This product is already in your cart!", "warning");
          }
        })
        .catch((error) => {
  ...
  if (filterCondition) filterCondition.addEventListener("change", applyFiltersAndSearch);
  if (filterCategory) filterCategory.addEventListener("change", applyFiltersAndSearch);
...
function applyFiltersAndSearch() {
  ...
  products.forEach((product) => {
    ...
    const conditionText = product.querySelector(".condition")?.textContent.toLowerCase() || "";
    const priceText = product.querySelector(".price")?.textContent || "$0";
    const price = parseFloat(priceText.replace("$", "").replace(",", ""));
    const productCategory = product.getAttribute("data-category")?.toLowerCase() || "";

    const matchesSearch = title.includes(searchTerm);
    const matchesCondition = condition === "all" || conditionText.includes(condition);
...
function resetFilters() {
  ...
  if (document.getElementById("filter-condition")) document.getElementById("filter-condition").value = "all";
  ...
  if (document.getElementById("sort-price")) document.getElementById("sort-price").value = "newest";
